...
#+begin_src sh
  sudo snap install emacs --classic &&
      sudo apt -y install cmake libtool-bin #=> for vterm
#+end_src
...
#+begin_src sh
  IDEAVERSION=2025.3.3 &&
      sudo rm -rf /usr/local/idea &&
      sudo mkdir -p /usr/local/idea &&
      wget -q -O- /tmp/idea.tar.gz https://download.jetbrains.com/idea/idea-${IDEAVERSION}.tar.gz | sudo tar xzvf - --strip-components=1 -C /usr/local/idea
#+end_src
...
PyPi上のものは最新版ではないので、Gitから直接インストールする
またapi-clientについてはPRからインストールする (AtCoderのメモリ表記変更が本体に反映されていないため)
https://github.com/online-judge-tools/api-client/pull/173
#+begin_src sh
  pip install git+https://github.com/online-judge-tools/oj.git
  pip install -U --force-reinstall "online-judge-api-client @ git+https://github.com/online-judge-tools/api-client.git@refs/pull/173/merge"
#+end_src
*** yq
#+begin_src sh
